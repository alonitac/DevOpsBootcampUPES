...
if [ -z "$PRIVATE_IP" ]; then
  ...
  if [ -z "$COMMAND" ]; then
    ssh -i "$KEY_PATH" ubuntu@"$BASTION_IP" ssh -tt -i key_pair.pem  ubuntu@"$PRIVATE_IP"
  else
    # Run a command on the private instance
    ssh -i "$KEY_PATH" ubuntu@"$BASTION_IP" ssh -tt -i key_pair.pem ubuntu@"$PRIVATE_IP" "$COMMAND"
  fi
fi
