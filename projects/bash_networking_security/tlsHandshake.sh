...
PUBLIC_INSTANCE_IP=13.52.239.222
...
server_hello=$(curl -X POST -H "Content-Type: application/json" -d "$client_hello" http://"$PUBLIC_INSTANCE_IP":8080/clienthello)
...
key_exchange_response=$(curl -X POST -H "Content-Type: application/json" -d "$key_exchange" http://"$PUBLIC_INSTANCE_IP":8080/keyexchange)
